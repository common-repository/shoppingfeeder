...
//            $product_data['sale_price'] = $product->min_variation_price;
	...
	private function get_product_data( $product, $price_currency = null, $price_currency_rate = null, $overrideCurrencyConversion = false )
	{
		...
		if ( !isset( $gtin ) || empty( $gtin ) )
		{
			//support for ICECAT produict_meta (and any other plugin that saves this meta)
			$gtin = get_post_meta($product->get_id(), 'product_ean', true);
			if ( empty( $gtin ) )
			{
				$gtin = null;
			}
		...
		if (empty($gtin))
		{
			if ( defined('WC_GLA_VERSION') )
			{
				try
				{
					$wc = new \Automattic\WooCommerce\GoogleListingsAndAds\Proxies\WC();
					$wp = new \Automattic\WooCommerce\GoogleListingsAndAds\Proxies\WP();
					global $wpdb;
					$table = new \Automattic\WooCommerce\GoogleListingsAndAds\DB\Table\AttributeMappingRulesTable($wp, $wpdb);
					$rc = new \Automattic\WooCommerce\GoogleListingsAndAds\DB\Query\AttributeMappingRulesQuery($wpdb,$table);
					$attribute_manager = new \Automattic\WooCommerce\GoogleListingsAndAds\Product\Attributes\AttributeManager($rc, $wc);
